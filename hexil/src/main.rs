...
impl smithay_client_toolkit::shell::xdg::window::WindowHandler for Application {
	fn request_close(
		&mut self,
		conn: &client::Connection,
...
fn wayland_context() -> Result<client::EventQueue<Application>, anyhow::Error> {
	let connection = client::Connection::connect_to_env()?;
	let (globals, mut queue) = client::globals::registry_queue_init::<Application>(&connection)?;
